...
class QueueServer:
    ...
    def do_process(self, abandon):
        if abandon:
            ...
            yield self.env.timeout(max(service_time_min, random.normalvariate(service_time, service_time_std)))
...
def QueueCustomer(env, customer_id: int, queue_server):
    ...
    with queue_server.queue_server.request() as request:
        ...
        #save the data
        customer_log.append((customer_id, enqueue_time, start_time, finish_time, len(queue), abandon))
...
def is_impatient(queue_time, random_number, queue_time_thresh=5, impatient_prob=0.2): # can later include more parameters (eg. hour of day)
    """
    determine if a user is impatient and leaves the queue before reaching an agent
    """
...
customer_df = pd.DataFrame(customer_log, columns=['customer_id','enqueue_time','start_time','finish_time','queue_length_upon_exit','is_abandoned'])
